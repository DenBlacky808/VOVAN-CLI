...
def cmd_worker(worker_mode: str, once: bool) -> int:
    settings = load_settings()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    mode.add_argument("--dry-run", dest="worker_mode", action="store_const", const="dry-run", default="from-env")
    mode.add_argument("--live", dest="worker_mode", action="store_const", const="live")
    p_worker.add_argument("--once", action="store_true", help="Execute a single claim/download/ocr/submit cycle")
...
def main() -> int:
    ...
    if args.command == "worker":
        return cmd_worker(args.worker_mode, args.once)
