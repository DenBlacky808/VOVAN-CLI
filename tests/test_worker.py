...
def test_worker_full_mocked_flow_complete(monkeypatch, tmp_path: Path) -> None:
    ...
    assert result["ocr"]["engine_requested"] == "placeholder"
    assert result["complete_result"] == {"ok": True}
    assert call_order == ["claim", "download", "complete", "status"]


def test_worker_tesseract_engine_falls_back_with_metadata(monkeypatch, tmp_path: Path) -> None:
    from vovan import worker as worker_module
    ...
    assert result["status"] == "ok"
    assert result["ocr_engine_configured"] == "tesseract"
    assert result["ocr_engine_used"] == "placeholder"
    assert result["ocr"]["engine_fallback"] is True
    assert "not installed yet" in result["ocr"]["engine_message"]
